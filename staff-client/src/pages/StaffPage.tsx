...
function StaffPage() {
	...
	const [staffActivationCode, setStaffActivationCode] =
		useState<StaffActivation | null>(null);
	...
	async function resetStaff(e: React.FormEvent) {
		e.preventDefault();
		if (!selectedStaff) return;
		const staffId = selectedStaff.id;
		try {
			const responseJson = await postRequest<StaffActivation>(
				"/staff/" + staffId + "/reset",
			);
			setModalType(null);
			fetchStaff();
			toast.success(responseJson.message);
			setStaffActivationCode(responseJson.data);
		} catch (error) {
			if (error instanceof Error) {
				toast.error(error.message);
			} else {
	...
	return (
		<>
			...
			{modalType === "reset" && selectedStaff && (
				<Modal
					title="Confirm Staff Activation Reset"
					onClose={() => {
						setModalType(null);
						setSelectedStaff(null);
					}}
				>
					<div className="flex flex-col gap-4">
						<div>
							Are you sure you want to reset activation for{" "}
							{selectedStaff.username}? This action cannot be undone.
						</div>
						<div className="flex flex-row-reverse gap-2">
							<Button onClick={resetStaff}>Confirm</Button>
							<Button
								variant="secondary"
								onClick={() => {
									setModalType(null);
			...
			{staffActivationCode && (
				<Modal
					title={"New Staff Activation Code"}
					onClose={() => {
						setStaffActivationCode(null);
					}}
				>
					<div className="flex flex-col gap-4">
						<div>
							A new activation code has been created for{" "}
							{staffActivationCode.username}. If the staff member was previously
							active, they will need this code to reactive their account.
						</div>
						<div className="text-xl">
							Activation Code: {staffActivationCode.activationCode}
						</div>
						<div className="flex flex-row-reverse gap-2">
							<Button
								variant="secondary"
